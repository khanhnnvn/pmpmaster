...
const IntegrationCard = ({ icon, iconBg, iconColor, title, desc, status, statusColor, btnLabel, primary }: any) => (
  <div className={`flex flex-col justify-between gap-4 rounded-xl bg-white dark:bg-card-dark p-5 shadow-sm border ${primary ? 'ring-2 ring-primary ring-offset-2 dark:ring-offset-background-dark border-transparent' : 'border-slate-100 dark:border-gray-800'} transition-all hover:shadow-md`}>
    ...
    <button className={`w-full mt-2 cursor-pointer flex items-center justify-center rounded-lg h-10 px-4 font-medium transition-all ${
      primary ? 'bg-primary text-white hover:bg-primary-hover shadow-md shadow-blue-500/20' : 'bg-primary/10 text-primary hover:bg-primary/20'
    }`}>
      {btnLabel}
    </button>
  </div>
...
const InputField = ({ label, type, value, placeholder, icon }: any) => (
  <div className="flex flex-col gap-2">
    ...
    <div className="relative">
      ...
      <input 
        className="block w-full pl-10 pr-10 rounded-lg border-slate-300 dark:border-gray-700 bg-slate-50 dark:bg-slate-800 text-sm h-11 focus:ring-primary focus:border-primary" 
        type={type} 
        defaultValue={value} 
