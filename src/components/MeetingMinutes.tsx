...
const MeetingItem = ({ active, date, month, title, desc, onClick }: {
    active: boolean;
    date: string;
    month: string;
    title: string;
    ...
    <div
        onClick={onClick}
        className={`group flex items-center gap-3 p-3 rounded-lg cursor-pointer border transition-all ${active ? 'bg-primary/5 border-primary/20' : 'hover:bg-slate-50 dark:hover:bg-slate-800/50 border-transparent'
            }`}
    >
        <div className={`flex flex-col items-center justify-center rounded-md h-12 w-12 shrink-0 border ${active ? 'bg-white dark:bg-slate-800 border-slate-100' : 'bg-slate-100 dark:bg-slate-800 border-transparent'
            }`}>
...
const Badge = ({ icon, text, isLink }: { icon: string; text: string; isLink?: boolean }) => (
...
const ActionItem = ({ text, user, linked }: { text: string; user: string; linked?: boolean }) => (
    <tr className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
        <td className="px-4 py-3 font-medium text-[#111418] dark:text-white">{text}</td>
        <td className="px-4 py-3">
            <div className="flex items-center gap-2">
                <div className="w-6 h-6 rounded-full bg-slate-200 overflow-hidden">
                    <img src="https://picsum.photos/id/10/24/24" className="w-full h-full object-cover" alt={user} />
                </div>
            ...
            <div className="w-full lg:w-80 flex flex-col bg-white dark:bg-card-dark rounded-xl shadow-sm border border-slate-200 dark:border-gray-800 shrink-0 overflow-hidden">
                <div className="p-4 border-b border-slate-100 dark:border-gray-800">
                    <div className="relative">
                        ...
                        <input
                            className="w-full pl-10 pr-4 py-2 bg-slate-50 dark:bg-slate-800 border-none rounded-lg text-sm focus:ring-2 focus:ring-primary/50 text-[#111418] dark:text-white outline-none"
