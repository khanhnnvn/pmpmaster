...
const TabButton = ({ active, onClick, label, badge }: {
    ...
    <button
        onClick={onClick}
        className={`px-4 pb-3 pt-2 font-bold text-sm whitespace-nowrap transition-all flex items-center gap-2 border-b-[3px] ${active ? 'border-primary text-text-main dark:text-white' : 'border-transparent text-text-muted hover:text-text-main'
            }`}
    >
        {label}
...
const TaskRow = ({ title, code, user, date, timeRemain }: {
    title: string;
    code: string;
    user: string;
    date: string;
    ...
    <tr className="group hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors cursor-pointer">
        ...
        <td className="px-6 py-4">
            <div className="flex items-center gap-2">
                <img src={`https://picsum.photos/id/${Math.floor(Math.random() * 50)}/32/32`} className="w-7 h-7 rounded-full object-cover" alt={user} />
        ...
        <div className="flex flex-col gap-8">
            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
                <div className="flex flex-col gap-2">
                    ...
                    <p className="text-sm text-text-muted">ID Dự án: #{id}</p>
                </div>
