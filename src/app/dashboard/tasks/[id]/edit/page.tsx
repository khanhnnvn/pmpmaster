...
export default function EditTaskPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const [taskRes, projectsRes, membersRes] = await Promise.all([
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.success) {
        const redirect = searchParams.get('redirect') || '/dashboard/tasks';
  ...
  return (
    <div className="flex flex-col gap-8 max-w-2xl">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-bold text-[#111418] dark:text-white">Chỉnh sửa công việc</h1>
          <p className="text-slate-500 dark:text-slate-400 text-sm">Cập nhật thông tin công việc</p>
        </div>
      </div>
